...
<#--                        :style="{width:''}"-->
<#--                        :disabled="false"-->
<#--                        :data="{uploadPath:'/banner','isRename':true,'appId':true}"-->
...
<#--                </el-upload>-->
<#--            </el-form-item>-->
<#--            <el-form-item  label="商务微信" prop="swwx">-->
<#--                <el-upload-->
<#--                        :file-list="form.swwx"-->
<#--                        :action="ms.base+'/file/upload.do'"-->
<#--                        :on-remove="swwxhandleRemove"-->
<#--                        :style="{width:''}"-->
<#--                        :limit="1"-->
<#--                        :on-exceed="appLogohandleExceed"-->
<#--                        :disabled="false"-->
<#--                        :data="{uploadPath:'/swwx','isRename':true,'appId':true}"-->
<#--                        :on-success="swwxSuccess"-->
<#--                        accept="image/*"-->
<#--                        list-type="picture-card">-->
<#--                    <i class="el-icon-plus"></i>-->
<#--                    <div slot="tip" class="el-upload__tip">支持jpg,png格式，最多上传1张图片</div>-->
...
<#--                        :file-list="form.gzh"-->
<#--                        :action="ms.base+'/file/upload.do'"-->
<#--                        :on-remove="gzhhandleRemove"-->
<#--                        :style="{width:''}"-->
<#--                        :limit="1"-->
<#--                        :on-exceed="appLogohandleExceed"-->
<#--                        :disabled="false"-->
<#--                        :data="{uploadPath:'/gzh','isRename':true,'appId':true}"-->
<#--                        :on-success="gzhSuccess"-->
<#--                        accept="image/*"-->
<#--                        list-type="picture-card">-->
<#--                    <i class="el-icon-plus"></i>-->
<#--                    <div slot="tip" class="el-upload__tip">支持jpg,png格式，最多上传1张图片</div>-->
...
<script>
    var form = new Vue({
        ...
        methods: {
            save: function () {
                var that = this;
                url = ms.manager + "/cms/app/update.do";
                this.$refs.form.validate(function(valid) {
                    if (valid) {
                        ...
                        if(data.swwx){
                            data.swwx = JSON.stringify(data.swwx);
                        }
                        if(data.gzh){
                            data.gzh = JSON.stringify(data.gzh);
                        }
            ...
            get: function (id) {
                var that = this;
                this.loading = true
                ms.http.get(ms.manager + "/cms/app/"+id+"/get.do", {"id":id}).then(function (res) {
                    that.loading = false
                    if(res.result && res.data){
                        ...
                        if(res.data.banner){
                            res.data.banner = JSON.parse(res.data.banner);
                            res.data.banner.forEach(function(value){
                                value.url= ms.base + value.path
                            })
                        }else{
                            res.data.banner=[]
                        ...
                        if(res.data.swwx){
                            res.data.swwx = JSON.parse(res.data.swwx);
                            res.data.swwx.forEach(function(value){
                                value.url= ms.base + value.path
                            })
                        }else{
                            res.data.swwx=[]
                        ...
                        if(res.data.gzh){
                            res.data.gzh = JSON.parse(res.data.gzh);
                            res.data.gzh.forEach(function(value){
                                value.url= ms.base + value.path
                            })
                        }else{
                            res.data.gzh=[]
            ...
            bannerhandleRemove:function(file, files) {
                var index = -1;
                index = this.form.banner.findIndex(function(e){return e == file} );
                if (index != -1) {
                    this.form.banner.splice(index, 1);
                }
            },
            swwxhandleRemove:function(file, files) {
                var index = -1;
                index = this.form.swwx.findIndex(function(e){return e == file} );
                if (index != -1) {
                    this.form.swwx.splice(index, 1);
                }
            },
            gzhhandleRemove:function(file, files) {
                var index = -1;
                index = this.form.gzh.findIndex(function(e){return e == file} );
                if (index != -1) {
                    this.form.gzh.splice(index, 1);
                }
            },
            ...
            bannerSuccess: function (response, file, fileList) {
                if(response.result){
                    this.form.banner.push({url:file.url,name:file.name,path:response.data,uid:file.uid});
                }else {
                    this.$notify({
                        title: '失败',
                        message: response.msg,
            ...
            swwxSuccess: function (response, file, fileList) {
                if(response.result){
                    this.form.swwx.push({url:file.url,name:file.name,path:response.data,uid:file.uid});
                }else {
                    this.$notify({
                        title: '失败',
                        message: response.msg,
            ...
            gzhSuccess: function (response, file, fileList) {
                if(response.result){
                    this.form.gzh.push({url:file.url,name:file.name,path:response.data,uid:file.uid});
                }else {
                    this.$notify({
                        title: '失败',
                        message: response.msg,
