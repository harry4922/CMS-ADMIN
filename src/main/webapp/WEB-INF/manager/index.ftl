            ...
            <el-main class="ms-admin-main">
                ...
                <el-tabs class="ms-admin-tabs" v-model="currentTab" type="card" closable @tab-remove="closeTab"
                    ...
                    <el-tab-pane v-for="(item, index) in editableTabs" :key="index" :label="item.modelTitle"
                        ...
                        <keep-alive>
                            <iframe :src='item.isStore?item.modelUrl:ms.manager+"/"+item.modelUrl+(item.modelUrl.indexOf("?")==-1?"?":"&")'
                                    :ref="item.modelTitle" style="background:url('${base}/static/images/loading.gif') no-repeat center;"></iframe>
                        </keep-alive>
                    </el-tab-pane>
                </el-tabs>
            </el-main>
...
<script>
    var indexVue = new Vue({
        ...
        data: {
            ...
            editableTabsValue: '',
            messageTypeList: [],
            editableTabs: [{"modelTitle": "文章管理", "modelUrl": "/cms/content/index.do"}], //当前打开的tab页面
            shortcutMenu: false, //快捷菜单显示状态
            collapseMenu: false, //菜单折叠，false不折叠
            currentTab: '文章管理', //当前激活tab的name
            tabIndex: 1,
            markList: [
